...
const inputClassName =
  'w-full rounded-[22px] border border-line bg-white px-5 text-lg text-cocoa outline-none transition placeholder:text-muted/60 focus:border-cocoa min-h-16';

function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-2 block text-sm font-bold uppercase tracking-[0.18em] text-muted">{label}</span>
      {children}
